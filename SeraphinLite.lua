-- Seraphin Lite v1.0 - Premium Minimal Dark UI
...
function Seraphin.CreateWindow(config)
    ...
    PageContainer.Position = UDim2.new(0, 122, 0, 5)
    ...
    ResizeHandle.ImageColor3 = THEME.TextMuted
    ResizeHandle.ImageTransparency = 0.6
    ResizeHandle.ZIndex = 10
    ResizeHandle.Parent = Main
    makeResizable(ResizeHandle, Main, Vector2.new(380, 280))
    ...
    function Window:SaveConfig(name)
        ...
        if not safeFolder("SeraphinConfigs") then safeMakeFolder("SeraphinConfigs") end
        safeWrite("SeraphinConfigs/" .. name .. ".json", HttpService:JSONEncode(data))
    end

    function Window:LoadConfig(name)
        local raw = safeRead("SeraphinConfigs/" .. name .. ".json")
        if raw then
    ...
    function Window:Tab(name, icon)
        ...
        function Tab:Section(text)
            ...
            Header.MouseButton1Click:Connect(function()
                ...
                UpdateSize()
                if not expanded then task.delay(0.25, function() if not expanded then Content.Visible = false end end) end
            end)

            local Sec = {}
            for k, v in pairs(Tab) do Sec[k] = v end
        ...
        function Tab:Button(text, callback)
            ...
            return {Frame = BFrame, SetText = function(t) Btn.Text = t end}
        end
        ...
        function Tab:Dropdown(text, options, default, callback, flag)
            ...
            Refresh()
            Events[flag] = {Set = SetOption}

            DBtn.MouseButton1Click:Connect(function()
                isOpen = not isOpen
                local h = isOpen and math.min(#options * 28 + 38, 160) or 32
            ...
            return {Frame = DFrame, Set = SetOption, Refresh = Refresh}
        end
        ...
        function Tab:MultiDropdown(text, options, default, callback, flag)
            ...
            return {Frame = DFrame, Set = SetOptions, Refresh = Refresh, Clear = function() selected = {} UpdateFlags() Refresh() end}
        end

        -- TextInput
        function Tab:TextInput(text, placeholder, callback, flag)
            flag = flag or text
            local default = Flags[flag] or ""
            Flags[flag] = default
            ...
            Events[flag] = {Set = SetText}
            IBox.FocusLost:Connect(function()
                Flags[flag] = IBox.Text
        ...
        function Tab:InputText2(text, placeholder, callback, flag)
            ...
            IBox.Text = default
            IBox.PlaceholderText = placeholder or "..."
            IBox.TextColor3 = THEME.TextMuted
            IBox.PlaceholderColor3 = THEME.TextMuted
            IBox.FontFace = THEME.Font
            IBox.TextSize = 11
